...
//【練習　一番下から上へスクロール 】
// リンクの要素を取得
document.querySelectorAll(".backToTop").forEach(function (btn) {
  //ForEach - すべての配列の要素に順番に処理を実行
...
document.getElementById("purpose").addEventListener("click", function (e) {
  const purposeTop = document.getElementById("purpose").offsetTop - 20;
  window.scrollTo({
    top: purposeTop, //上まで
    behavior: "smooth", // なめらかに
  });
});
...
// リンクの要素を取得
document
  .querySelectorAll("nav a") //navの中のa要素をすべて取得
  .forEach((link) => {
    link //すべての配列（今回はNav a)の要素に順番に処理を実行
      //ボタンがクリックされたイベントハンドラ
      .addEventListener("click", function (e) {
        e.preventDefault(); //←HTML側のリンクの本来の機能をキャンセル

        const href = //"#profile"→hrefを定義
          //this→いまイベント中の要素を指定できる→今回はクリックした aタグの代わり
          this.getAttribute("href"); // HTMLタグの属性の文字列を取得する → "#profile"取得→上へ戻る
        //href="#profile"（文字列）なので、↓で要素（DOM）に直す
        const target = document.querySelector(href); // IDセレクタとして使う
        // これで <section id="profile">自体を要素として取り出せた

        if (target) {
          //<section id="profile">が変数
          const topPosition = target.offsetTop - 20;
          //  <section id="profile">から垂直距離

          //ウインドウをスクロールさせる
          window.scrollTo({
            // スクロールの仕方についてのオブジェクト
            top: topPosition, //ページを縦にその位置までスクロールする
            behavior: "smooth", // なめらかに
          });
        }
      });
...
//todo 2.ユーザーに合わせてダークモードの切り替え　→ 調べた結果CSSで実装
//1.ユーザーのモードを読み取る
//2.ユーザー情報に合わせて、ライト/ダークモード切り替える
...
//todo 3.プロフィール欄の「もっと見る」開閉
// スクロールでも見れるほうが見やすいので中止
// 代わりに自動スクロール機能を追加

//todo 4.SNSアイコンをボタン風にする
// CSSで実装
